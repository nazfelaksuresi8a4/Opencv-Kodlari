    ...
    if ptype == 'img':
                                    ...
                                    if calibartionMetadatas is not None:
                                        if isinstance(calibartionMetadatas,tuple):
                                            if len(calibartionMetadatas) == 2:
                                                ...
                                                line_y = cv.line(nii_matrix.copy(),(y,yQ),(y,0),(255,255,255),thickness=2)
                                                result_matlike = cv.line(line_y,(xQ,x),(0,x),(255,255,255),thickness=2,lineType=1)
        ...
        if result_matlike is not None:
            if len(result_matlike.shape) == 2:
                plt.imshow(result_matlike,cmap='gray')
                plt.show()
